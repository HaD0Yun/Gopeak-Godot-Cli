...
[![](https://img.shields.io/github/last-commit/HaD0Yun/Gopeak-godot-Cli 'Last Commit')](https://github.com/HaD0Yun/Gopeak-godot-Cli/commits/dev)
...
`gopeak-cli` is the package and command you install; it packages the `godot-flow` 3-layer MCP architecture for AI-assisted Godot automation. Born from [GoPeak (godot-mcp)](https://github.com/HaD0Yun/godot-mcp), it compresses 220 individually-registered MCP tools into 4 meta-tools — a **54× token reduction** (measured via actual JSON-RPC `tools/list` responses). Adding functions costs zero extra tokens.

## Naming Map

| Surface | Name | What to use it for | Compatibility note |
|---|---|---|---|
| Repository | `Gopeak-godot-Cli` | GitHub repo, clone URL, issue tracker | Keep as repo identity |
| npm package | `gopeak-cli` | Package name used by builds and install metadata | Install from GitHub until npm publish |
| CLI command | `gopeak-cli` | Run `gopeak-cli listfunc`, `gopeak-cli exec`, etc. | Stable command; avoid renaming in scripts |
| MCP server display name | `godot-flow` | Name some MCP clients may show after connection | Preserved for compatibility in this PR |
| Environment variables | `GODOT_FLOW_*` | Configure project path, Godot path, ports, timeout | Preserved for compatibility with existing setups |

> **Recommended public-facing name:** use **`gopeak-cli`** when telling users what to install and run. Treat **`godot-flow`** as the MCP architecture/server identity that remains on protocol-facing surfaces.
...
| Problem with traditional MCP | gopeak-cli / godot-flow solution |
|---|---|
...
```bash
npx -y github:HaD0Yun/Gopeak-godot-Cli#dev listfunc
```

### Global Install

```bash
npm install -g github:HaD0Yun/Gopeak-godot-Cli#dev
gopeak-cli listfunc
```
...
Use `gopeak-cli` as the command in client config examples. The JSON object key is just a local label, so the docs use `gopeak-cli` for consistency. Some clients may still display the server metadata name `godot-flow` after connection because that protocol-facing identifier is intentionally left unchanged for compatibility.
...
The `gopeak-cli` command mirrors the MCP tools for terminal use and debugging:
...
The CLI command is `gopeak-cli`, but the environment variables intentionally remain `GODOT_FLOW_*` for backward compatibility with existing MCP and shell setups.
...
`gopeak-cli` can install shell hooks that detect supported AI CLI commands already present on your machine and wrap them with cached update + GitHub star prompts. Current installs still run a silent postinstall setup step, so `npm install` or GitHub-based install may create or update your shell rc file automatically. Use `gopeak-cli uninstall` to remove those hooks, or rerun `gopeak-cli setup` later if you want to refresh them deliberately.
